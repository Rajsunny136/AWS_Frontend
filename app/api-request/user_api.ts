...
export const PaymentScreen = async (id: number, active: boolean) => {
    try {
        const token = await getAuthToken();
        const response = await axios({
            method: 'post',
            url: `${origin}/api/v1/user/transactions/`,
            headers: {
                "Content-Type": "application/json",
                "Authorization": `Bearer ${token}`
            },
...
//user wallet getroute

export const getPaymentScreen = async (id: number, active: boolean) => {
    try {
        const token = await getAuthToken();
        const response = await axios({
            method: 'get',
            url: `${origin}/api/v1/user/transactions/:user_id`,
            headers: {
                "Content-Type": "application/json",
                "Authorization": `Bearer ${token}`
            },
